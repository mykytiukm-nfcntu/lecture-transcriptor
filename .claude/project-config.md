...
- **Pipeline is not resumable.** Any failure sets `lecture.status = failed` and populates `error_message`. Do not add checkpointing, retry-from-stage, or partial-artifact reuse.
